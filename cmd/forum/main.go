			...
			for _, v := range config.TelegramChatID {
				for _, guild := range v {
					if guild == m.GuildID.String() {
						...
						sendMessageToTelegram(config, telegram, Message{Content: m.Content, ChannelName: channel.Name, GuildName: guild.Name, GuildID: m.GuildID.String()})
					}
				}
			}
		}
